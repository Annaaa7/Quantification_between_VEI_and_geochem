...
results_polr <- list()
failed_polr  <- data.frame(Rep=integer(), Fold=integer(), reason=character(), stringsAsFactors = FALSE)

for (split in all_splits_summary) {
  train <- split$train
  test  <- split$test

  # Réponse ordonnée et niveaux cohérents
  train$New_Vei <- factor(train$New_Vei, ordered = TRUE)
  test$New_Vei  <- factor(test$New_Vei,  levels = levels(train$New_Vei), ordered = TRUE)

  # Skip si trop peu de classes/eff. faibles
  if (length(levels(train$New_Vei)) < 3 || any(table(train$New_Vei) < 3)) next

  # Formule (ou tes colonnes nettoyées)
  polr_formula <- as.formula(paste("New_Vei ~", paste(X_mean_sd_cols, collapse = " + ")))

  fit <- tryCatch(
    MASS::polr(polr_formula, data = train, method = "logistic",
               Hess = TRUE, control = list(maxit = 200)),
    error = function(e) {
  ...
  if (is.null(fit)) next  # on saute ce split et on continue la boucle

  # --- suite normale : probs, coûts, préd, accuracy ---
  probs_polr <- predict(fit, newdata = test, type = "probs")
  ...
  pred  <- factor(levs[max.col(probs_polr)], levels = levs, ordered = TRUE)
  ...
  results_polr[[length(results_polr) + 1]] <- data.frame(
    Rep = split$rep, Fold = split$fold,
    C_absolute = costs$C_absolute, C_squared = costs$C_squared,
    C_asymmetric = costs$C_asymmetric, Weighted_Accuracy = wa
  )
}
...
# Tout en un seul data.frame
results_polr_df <- if (ok_n) do.call(rbind, results_polr) else data.frame()
...
## --- (A) Recompute a clean prediction ---
pred_orf <- predict(model_orf, newdata = as.data.frame(test_orf))

## Sanity checks (must be TRUE / FALSE as below)
print(class(pred_orf))                      # should be "ordforpred"
print(is.matrix(pred_orf$classprobs))       # should be TRUE
print(is.vector(pred_orf$ypred) || is.factor(pred_orf$ypred))  # should be TRUE

## --- (B) Extract flat objects ONLY ---
pred_orf_vec <- as.character(pred_orf$ypred)

probs_orf_df <- as.data.frame(pred_orf$classprobs, check.names = FALSE)
colnames(probs_orf_df) <- as.character(seq_len(ncol(probs_orf_df)))  # "1","2","..."

probs_orf_df <- probs_orf_df[, as.character(1:6), drop = FALSE]

## --- (C) Build result: DO NOT include `pred_orf` anywhere ---
result_orf_aug <- bind_cols(
  tibble(
    Eruption        = eruption_ids,           # aligné avec test_orf[ok, ]
    Predicted_Class = pred_orf_vec
  ),
  probs_orf_df
)
